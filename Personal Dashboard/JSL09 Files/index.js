// Getting a background image for my dashboard
...
.then(data => {
  ...
  .catch(err => {
        // Setting a default background if there's issues with the API
...
// Getting current data on a cryptocurrency (ethereum)
...
navigator.geolocation.getCurrentPosition(position => {
    fetch(`https://apis.scrimba.com/openweathermap/data/2.5/weather?lat=${position.coords.latitude}&lon=${position.coords.longitude}&units=metric`)
        ...
        .catch(err => console.error(err)) // add this method so that it can catch the error if there's one.
});
